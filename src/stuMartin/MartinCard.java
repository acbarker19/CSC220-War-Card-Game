...
public class MartinCard extends helperCards.Card{       //creates a sub class of the Card class in the package helperCards (MartinCard is a subclass of Card)
    ...
    public MartinCard (String s, int v){
        super (s, v);
    }//end of 2-paramenter constructor
    
    public MartinCard (String suit, int value, int over, int down, BufferedImage image){
        super (suit, value, over, down, image);
    }//end of 5-parameter constructor
    
    public MartinCard (Card c){
        super(c.getSuit(), c.getValue(), c.getOver(), c.getDown(), c.getImage());
    }//end of constructor that has a parameter of a card object
    
     public boolean containsMousePoint(int x, int y){
        //gets the over value of the upper left-hand corner of the card.
        int cardOver = getOver() - width / 2;
        //gets the down value of the upper left-hand corner of the card.
        int cardDown = getDown() - height / 2;
        //Test to see if the mouse pointer is in a card on the GUI.
        if (x>cardOver && x<cardOver + width && y>cardDown && y <cardDown + height){
            return true;
        }//end of if-statement-returns true because the mouse is in a card
        else{
            return false;
        }//end of else-returns false because the mouse is not in a card
    }//end of containsMousePoint()
    
    public boolean containsGoal(int x, int y){
        //gets the over value of the upper left-hand corner of the card.
        int cardOver = getOver() - width / 2;
        //gets the down value of the upper left-hand corner of the card.
        int cardDown = getDown() - height / 2;
        //Test to see if the card is in the goal.
        if ((cardOver>=500 && cardOver<= 500+290) && (cardDown >= 300 && cardDown <= 300+140)){
            return true;
        }//end of if statement-returns true because the card is in the goal.
        else{
            return false;
        }//end of else-returns false because the card is not currently in the gial.
    }//end of containsGoal()

     public boolean isValidType(){
         if ((getValue()%2==1) || (getSuit().equals("King")) || (getSuit().equals("Queen"))){
             return true;
         }//end of if-statement-returns true because the card that is on the goal is a valid type.
         else{
             return false;
         }//end of else-returns false because the card that is on the goal is not a valid type.
     }//end of isValidType()
}//end of MartinCard class